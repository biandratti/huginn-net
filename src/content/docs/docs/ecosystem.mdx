...
- Match tiers (`Specific` / `Generic` / `Fuzzy`) instead of a continuous score
...
**Purpose:** TLS ClientHello fingerprinting on **TCP** using the official JA4 specification (`t` prefix). QUIC and DTLS are on the roadmap.
...
| `stable-v1` | Adds `JA4_s1` / `JA4_rs1` (huginn-only): JA4 minus `S1_SESSION_EXTENSIONS` so resumption does not split the key |
| `json` | `serde::Serialize` on all output types |

**Key Features:**

- **TLS parsers:** ClientHello and record-level parsing (via `tls-parser`) for reliable JA4 inputs
- JA4 fingerprinting (FoxIO specification, TCP) — `JA4`, `JA4_r`, `JA4_o`, `JA4_ro`
- **JA4_s1 / JA4_rs1** — huginn stable variants; drop padding, session ticket, pre-shared key, and early data before hashing ([JA4S1.md](https://github.com/biandratti/huginn-net/blob/master/huginn-net-tls/JA4S1.md))
...
- Gate-based matching: a field that does not fit rejects the signature
- Match tiers: `Specific` / `Generic` / `Fuzzy` (TCP only); HTTP has no error budget
- `SharedTcpSignatureMatcher` / `SharedHttpSignatureMatcher` for use with `.with_matcher(...)`

How labels and `Params` are produced is documented under [Matching](../matching/).
