...
use tls_parser::{
    parse_tls_plaintext, TlsClientHelloContents, TlsMessage, TlsMessageHandshake,
    TlsVersion as RusticataTlsVersion,
};
use tracing::debug;
...
pub fn parse_tls_client_hello_rusticata(data: &[u8]) -> Result<TlsSignature, PassiveTcpError> {
    debug!(
        "RUSTICATA DEBUG: Starting TLS parsing with {} bytes",
        data.len()
    );
    debug!(
        "RUSTICATA DEBUG: First 64 bytes: {:02x?}",
    ...
    match parse_tls_plaintext(data) {
        Ok((remaining, tls_record)) => {
            ...
            debug!(
                "RUSTICATA DEBUG: TLS record has {} messages",
                tls_record.msg.len()
            );
            ...
            for (i, message) in tls_record.msg.iter().enumerate() {
                ...
                if let TlsMessage::Handshake(handshake) = message {
                    debug!(
                        "RUSTICATA DEBUG: Found handshake message: {:?}",
                    ...
                    if let TlsMessageHandshake::ClientHello(client_hello) = handshake {
                        ...
                        debug!(
                            "RUSTICATA DEBUG: ClientHello version: 0x{:04x}",
                            client_hello.version.0
                        );
                        debug!(
                            "RUSTICATA DEBUG: ClientHello ciphers: {}",
                            client_hello.ciphers.len()
                        );
                        debug!(
                            "RUSTICATA DEBUG: ClientHello extensions present: {}",
                            client_hello.ext.is_some()
                        );
                        if let Some(ext_data) = &client_hello.ext {
                            debug!(
                                "RUSTICATA DEBUG: Extension data size: {} bytes",
                                ext_data.len()
                            );
                        }
...
fn extract_tls_signature_from_client_hello(
    ...
    let expected_ciphers = vec![
        0x002f, 0x0035, 0x009c, 0x009d, 0x1301, 0x1302, 0x1303, 0xc009, 0xc00a, 0xc013, 0xc014,
        0xc02b, 0xc02c, 0xc02f, 0xc030, 0xcca8, 0xcca9,
    ];
    let expected_extensions = vec![
        0x0000, 0x0005, 0x000a, 0x000b, 0x000d, 0x0010, 0x0012, 0x0017, 0x001b, 0x001c, 0x0022,
        0x0023, 0x002b, 0x002d, 0x0033, 0xfe0d, 0xff01,
    ];

    // Compare results
    let missing_ciphers: Vec<_> = expected_ciphers
        .iter()
        .filter(|&c| !cipher_suites.contains(c))
        .collect();
    let extra_ciphers: Vec<_> = cipher_suites
        .iter()
    ...
    if !missing_ciphers.is_empty() {
        debug!(
            "RUSTICATA - Missing cipher suites: {:04x?}",
            missing_ciphers
        );
    }
    ...
    let missing_extensions: Vec<_> = expected_extensions
        .iter()
        .filter(|&e| !extensions.contains(e))
        .collect();
    let extra_extensions: Vec<_> = extensions
        .iter()
    ...
    if !missing_extensions.is_empty() {
        debug!(
            "RUSTICATA - Missing extensions: {:04x?}",
            missing_extensions
        );
    }
    ...
    debug!(
        "RUSTICATA - Cipher suites count: {} (expected: 17)",
        cipher_suites.len()
    );
    debug!(
        "RUSTICATA - Extensions count: {} (expected: 17)",
        extensions.len()
    );
    debug!(
        "RUSTICATA - Signature algorithms count: {} (expected: 11)",
        signature_algorithms.len()
    );
...
fn determine_tls_version(legacy_version: &RusticataTlsVersion, extensions: &[u16]) -> TlsVersion {
    ...
    debug!(
        "Determining TLS version: legacy_version=0x{:04x}, extensions={:04x?}",
        legacy_version.0, extensions
    );
    ...
    if tls13_ext_count >= 2 {
        debug!(
            "Found {} TLS 1.3 indicator extensions, assuming TLS 1.3",
            tls13_ext_count
        );
        return TlsVersion::V1_3;
    }
...
fn parse_extensions_from_client_hello(
    ...
    debug!(
        "RUSTICATA DEBUG: ClientHello has ext field: {:?}",
        client_hello.ext.is_some()
    );
    ...
    if let Some(ext_data) = &client_hello.ext {
        debug!(
            "RUSTICATA DEBUG: Extension data length: {} bytes",
            ext_data.len()
        );
        debug!(
            "RUSTICATA DEBUG: First 32 bytes of ext_data: {:02x?}",
            &ext_data[..std::cmp::min(32, ext_data.len())]
        );
        ...
        debug!(
            "RUSTICATA DEBUG: Parsed {} extensions from ext_data",
            extensions.len()
        );
    } else {
        ...
        debug!(
            "RUSTICATA DEBUG: ClientHello ciphers count: {}",
            client_hello.ciphers.len()
        );
        debug!(
            "RUSTICATA DEBUG: ClientHello random length: {}",
            client_hello.random.len()
        );
        debug!(
            "RUSTICATA DEBUG: ClientHello session_id: {:?}",
            client_hello.session_id.map(|s| s.len())
        );
        debug!(
            "RUSTICATA DEBUG: ClientHello comp count: {}",
            client_hello.comp.len()
        );
    }
...
fn parse_extensions_from_raw_detailed(
    ...
    debug!(
        "Parsing extensions from {} bytes of raw data: {:02x?}",
        ext_data.len(),
        &ext_data[..std::cmp::min(32, ext_data.len())]
    );
    ...
    debug!(
        "Processing extensions data, total available: {} bytes",
        extensions_end
    );
    ...
    while offset + 4 <= extensions_end {
        ...
        debug!(
            "Found extension: type=0x{:04x}, length={}, offset={}, end={}",
        ...
        match extension_type {
            ...
            0x000d => {
                ...
                if !parsed_sig_algs.is_empty() {
                    signature_algorithms = parsed_sig_algs;
                    debug!(
                        "Parsed {} signature algorithms: {:04x?}",
                        signature_algorithms.len(),
                        signature_algorithms
                    );
                }
            ...
            0x44cd => {
                debug!(
                    "PROBLEMATIC: Found extension 0x44cd (len={}), data: {:02x?}",
                    extension_length,
                    &extension_data[..std::cmp::min(16, extension_data.len())]
                );
            }
            0x0029 => {
                debug!(
                    "PROBLEMATIC: Found extension 0x0029 (pre_shared_key, len={}), data: {:02x?}",
                    extension_length,
                    &extension_data[..std::cmp::min(16, extension_data.len())]
                );
            }
            0x001c => {
                debug!(
                    "EXPECTED: Found extension 0x001c (cert_type, len={}), data: {:02x?}",
                    extension_length,
                    &extension_data[..std::cmp::min(16, extension_data.len())]
                );
            }
            0x0022 => {
                debug!(
                    "EXPECTED: Found extension 0x0022 (cert_status_type, len={}), data: {:02x?}",
                    extension_length,
                    &extension_data[..std::cmp::min(16, extension_data.len())]
                );
            }
            _ => {
                ...
                debug!(
                    "Extension 0x{:04x} recorded but not parsed in detail (len={})",
    ...
    debug!(
        "Parsed {} total extensions: {:04x?}",
        extensions.len(),
        extensions
    );
    debug!(
        "Expected {} extensions: {:04x?}",
    ...
    if !missing_extensions.is_empty() {
        debug!(
            "ANALYSIS: Missing expected extensions: {:04x?}",
            missing_extensions
        );
    }
    if !extra_extensions.is_empty() {
        debug!(
            "ANALYSIS: Extra extensions found: {:04x?}",
            extra_extensions
        );
    }
    ...
    debug!(
        "Signature algorithms: {} items: {:04x?}",
        signature_algorithms.len(),
        signature_algorithms
    );
    debug!(
        "Elliptic curves: {} items: {:04x?}",
        elliptic_curves.len(),
        elliptic_curves
    );
...
fn parse_signature_algorithms_extension(data: &[u8]) -> Vec<u16> {
    ...
    debug!(
        "Signature algorithms list length: {}, available data: {}",
        list_length,
        data.len() - 2
    );
...
fn parse_supported_groups_extension(data: &[u8]) -> Vec<u16> {
    ...
    debug!(
        "Supported groups list length: {}, available data: {}",
        list_length,
        data.len() - 2
    );
...
mod tests {
    ...
    fn test_grease_filtering() {
        ...
        let filtered: Vec<u16> = test_ciphers
            .into_iter()
            .filter(|&cipher| !GREASE_VALUES.contains(&cipher))
            .collect();
