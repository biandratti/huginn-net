...
```toml
[dependencies]
huginn-net = "2.1.0"  # Complete analysis suite
```

**For specific protocols:**
```toml
[dependencies]
huginn-net-tcp = "2.1.0"   # TCP/OS fingerprinting only
huginn-net-http = "2.1.0"  # HTTP analysis only
huginn-net-tls = "2.1.0"   # TLS/JA4 analysis only
huginn-net-db = { version = "2.1.0", features = ["tcp", "http"] }  # signature matching for TCP/HTTP (not needed for TLS)
```
