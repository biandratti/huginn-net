    ...
    if let Some(full) = full_analysis {
        ...
        if parallel_2.is_some() || parallel_4.is_some() || parallel_8.is_some() {
            ...
            let available_cpus = std::thread::available_parallelism()
                .map(|n| n.get())
                .unwrap_or(1);
            let parallel_efficiency = 0.85; // Assume 85% scaling efficiency for hash-based routing
            ...
            if let Some(p2) = parallel_2 {
                ...
                let speedup = throughput
                    / calculate_throughput(
                        full.checked_div(report.packet_count as u32)
                            .unwrap_or(Duration::ZERO),
                        1,
                    );
                println!("  2 Workers:");
            ...
            if let Some(p8) = parallel_8 {
                ...
                println!(
                    ...
                    if cpu_10gbps_parallel > 100.0 {
                        " [OVERLOAD - requires more workers]"
                    } else {
                        " [OK]"
                    }
                );
