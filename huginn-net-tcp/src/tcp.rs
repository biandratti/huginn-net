//! Protocol-level TCP types used for fingerprinting.
//!
//! These types represent raw TCP/IP header fields and are used both for
//! fingerprint extraction and (when the `db` feature is enabled) for
//! matching against the p0f signature database.
...
impl TcpMatchQuality {
    ...
    pub fn distance_to_score(distance: u32) -> f32 {
...
pub enum Ttl {
    ...
    /// First u8 is the observed TTL value, second u8 is the estimated hops.
    Distance(u8, u8),
    /// TTL value guessed based on common OS initial values (e.g. 64 for Linux, 128 for Windows).
    Guess(u8),
...
#[derive(Clone, Debug, PartialEq)]
pub enum WindowSize {
    /// Window size is a multiple of MSS.
    Mss(u8),
    /// Window size is a multiple of MTU.
    Mtu(u8),
    /// Raw window size value.
    Value(u16),
...
pub enum PayloadSize {
    /// Packet has no payload (empty). Common in SYN packets.
    Zero,
    /// Packet contains data in the payload.
    NonZero,
    /// Matches any payload size (wildcard).
    Any,
}
