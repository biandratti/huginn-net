...
impl OSQualityMatched {
    ...
    ///
    /// Order matches `dump_flags` (`fp_tcp.c:878-896`): `generic`, `fuzzy`,
    /// `random_ttl`, `excess_dist`, `tos:0xNN`. Fuzzy keeps the typed detail in
    /// parentheses; the richer reason also lives on [`MatchQuality::Matched`].
