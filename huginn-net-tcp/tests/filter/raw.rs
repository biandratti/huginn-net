...
fn build_raw_ipv4_packet_with_ihl(ihl: u8, src_port: u16, dst_port: u16) -> Vec<u8> {
    ...
    let total_len = ip_header_len.max(20).saturating_add(4); // at least 20 bytes IP + 4 bytes TCP ports
    let mut packet = vec![0u8; total_len];
    packet[0] = (4 << 4) | (ihl & 0x0F); // version=4, IHL=ihl
    packet[9] = 6; // TCP protocol
...
fn test_raw_filter_malformed_ihl_zero_is_rejected() {
    // IHL=0 → ip_header_len=0: without the guard, bytes 0-3 of the IP header
    // would be read as TCP ports, potentially bypassing port filters.
    let filter = FilterConfig::new()
        .mode(FilterMode::Deny)
        .with_port_filter(PortFilter::new().destination(443));

    let packet = build_raw_ipv4_packet_with_ihl(0, 12345, 443);
    assert!(huginn_net_tcp::raw_filter::apply(&packet, &filter));
}
...
fn test_raw_filter_valid_ihl_five_passes_non_matching_port() {
    // IHL=5, destination port 443 → deny-22 filter should allow it (returns true).
    let filter = FilterConfig::new()
        .mode(FilterMode::Deny)
        .with_port_filter(PortFilter::new().destination(22));
