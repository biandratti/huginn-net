...
fn test_worker_pool_rejects_zero_workers() {
    ...
    let result = WorkerPool::new(0, 100, 32, 10, tx, #[cfg(feature = "db")] None, 1000, None);
...
fn test_worker_pool_creates_with_valid_workers() {
    let (tx, _rx) = mpsc::channel();
    let result = WorkerPool::new(4, 100, 32, 10, tx, #[cfg(feature = "db")] None, 1000, None);
    assert!(result.is_ok());
...
fn test_hash_based_dispatch_consistency() {
    let (tx, _rx) = mpsc::channel();
    let queue_size = 10;
    let pool = unwrap_worker_pool(WorkerPool::new(4, queue_size, 32, 10, tx, #[cfg(feature = "db")] None, 1000, None));
...
fn test_queue_overflow_handling() {
    let (tx, _rx) = mpsc::channel();
    let queue_size = 5;
    let pool = unwrap_worker_pool(WorkerPool::new(2, queue_size, 32, 10, tx, #[cfg(feature = "db")] None, 1000, None));
...
fn test_stats_accuracy() {
    let (tx, _rx) = mpsc::channel();
    let pool = unwrap_worker_pool(WorkerPool::new(2, 100, 32, 10, tx, #[cfg(feature = "db")] None, 1000, None));
...
fn test_per_worker_dropped_count() {
    let (tx, _rx) = mpsc::channel();
    let queue_size = 2;
    let pool = unwrap_worker_pool(WorkerPool::new(1, queue_size, 32, 10, tx, #[cfg(feature = "db")] None, 1000, None));
...
fn test_concurrent_dispatch() {
    let (tx, _rx) = mpsc::channel();
    let pool = Arc::new(unwrap_worker_pool(WorkerPool::new(4, 100, 32, 10, tx, #[cfg(feature = "db")] None, 1000, None)));
...
fn test_state_isolation() {
    ...
    let queue_size = 5;
    let pool = unwrap_worker_pool(WorkerPool::new(3, queue_size, 32, 10, tx, #[cfg(feature = "db")] None, 1000, None));
