...
//!
//! p0f tries to `FATAL` when one signature would always win over another that
//! also fits (`fp_tcp.c` registration). Upstream's check is broken for many
//! entries, so we only `warn!` — and tests freeze the known list so new
//! ambiguities in our copy of `p0f.fp` show up in CI.
