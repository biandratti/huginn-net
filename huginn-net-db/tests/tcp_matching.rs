#![cfg(feature = "tcp")]
//! Gate behaviour of `Signature::fit`.
//!
//! p0f rejects a candidate signature outright when `olen`, `mss`, `wscale` or
//! the window size disagree; none of
//! them may degrade into a fuzzy match. These tests pin that contract on the
...
fn exact() -> Option<SignatureFit> {
    Some(SignatureFit::exact(0))
}

/// The match only holds because a tolerance was applied.
fn fuzzy() -> Option<SignatureFit> {
    Some(SignatureFit::fuzzy(0))
}
