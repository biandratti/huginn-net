...
// Since raw_filter functions are module-private, we test through the public API
...
fn create_ipv4_tcp_packet(
    ...
    // IP version and header length (version 4, IHL 5 = 20 bytes)
    packet[0] = 0x45;
    ...
    // TCP source port
    let src_bytes = src_port.to_be_bytes();
    packet[20..22].copy_from_slice(&src_bytes);

    // TCP dest port
    let dst_bytes = dst_port.to_be_bytes();
    packet[22..24].copy_from_slice(&dst_bytes);
...
fn create_ethernet_ipv4_tcp_packet(
    ...
    // EtherType: IPv4 (0x0800) at offset 12-13
    ...
    // TCP ports at offset 34 (14 Ethernet + 20 IP)
    let src_bytes = src_port.to_be_bytes();
    packet[34..36].copy_from_slice(&src_bytes);

    let dst_bytes = dst_port.to_be_bytes();
    packet[36..38].copy_from_slice(&dst_bytes);
...
fn test_raw_filter_combined_filters() {
    ...
    // Should fail port filter
    assert!(!filter.should_process(&src_ip, &dst_ip, 12345, 80));

    // Should fail IP filter
...
fn test_raw_filter_no_filter_allows_all() {
    ...
    // No filters configured, should allow everything
    assert!(filter.should_process(&src_ip, &dst_ip, 12345, 443));
