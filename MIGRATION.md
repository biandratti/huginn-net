...
**Crate:** [`huginn-net-tls`](huginn-net-tls). Same types if you consume TLS through
`huginn-net`. Other crates are unchanged.

### Fingerprint names (`json` / `Display`)

FoxIO order is raw-then-mode. The fingerprint string did not move; only the
key. `stable-v1` / `tls-stable-v1` follows the same order.

| | v2.1 | v2.2 |
|--|------|------|
| JSON | `ja4_or` | `ja4_ro` |
| Display | `JA4_or` | `JA4_ro` |
| JSON (`stable-v1`) | `ja4_s1r` | `ja4_rs1` |
| Display (`stable-v1`) | `JA4_s1r` | `JA4_rs1` |

`ja4`, `ja4_r`, `ja4_o`, `ja4_s1` unchanged.

### `JA4_s1` (`stable-v1`)

2.1 dropped padding `0x0015`, session_ticket `0x0023`, PSK `0x0029`. 2.2 uses
`S1_SESSION_EXTENSIONS` and also drops `early_data` `0x002a`. Official JA4
is unchanged. Stored s1 keys change only if you see 0-RTT.

`EPHEMERAL_TLS_EXTENSIONS` → `S1_SESSION_EXTENSIONS`. List:
[`huginn-net-tls/JA4S1.md`](huginn-net-tls/JA4S1.md).

Local extra IDs (workaround until they land on that list):
`Signature::generate_ja4_stable_v1_excluding` /
`HuginnNetTls::with_s1_excluded_extensions`. Empty = canonical.
`process_ipv4_packet` / `process_ipv6_packet` take `&[u16]`; `WorkerPool::new`
takes `Arc<[u16]>` (`&[]` / `Arc::from([])`).

### GREASE in signature algorithms

FoxIO now strips GREASE from `signature_algorithms`. 2.2 matches; Chromium /
Google JA4 values change. Chromium v152 initial:
...
Highest non-GREASE `supported_versions`; else `legacy_version`. Unknown → `00`.

```rust
determine_tls_version(legacy, &[0x002b])                    // v2.1
